...
1.	Use your preferred code editor to add a new **Console App** / `console` project named `GeneratingCodeApp` to the `Chapter07` solution.
2.	In `GeneratingCodeApp.csproj`, statically and globally import the `Console` class, as shown in the following markup:
```xml
<ItemGroup>
...
2.	In `GeneratingCodeApp.csproj`, add a reference to the class library project, as shown in the following markup:
```xml
<ItemGroup>
...
3.  Build the `GeneratingCodeApp` project.
4.  Note the auto-generated class file:
- In Visual Studio 2022, close and reopen the solution to trigger analyzers to be run, and then in **Solution Explorer**, expand the **Dependencies** | **Analyzers** | **GeneratingCodeLib** | **Packt.Shared.ConfigureConsoleSourceGenerator** nodes, and then open the `Program.Methods.g.cs` file, as shown in Figure 7B.1:
...
- In Visual Studio Code, in **EXPLORER** | **CHAPTER07**, in the `obj/Debug/net8.0` folder, note the `generated` folder and its subfolder `GeneratingCodeLib/Packt.Shared.ConfigureConsoleSourceGenerator`folder, and then open the `Program.Methods.g.cs` file, as shown in *Figure 7B.2*:
...
7.  Run the `GeneratingCodeApp` project, and note the result, as shown in the following output:
```
...
9.  Run the `GeneratingCodeApp` project, and note the result, as shown in the following output:
```
...
11.   Run the `GeneratingCodeApp` project, and note the result, as shown in the following output:
```
